...
private:
  // ---------------------------------------------------------------------------
  // Cost model helper: rough per-iteration cost based on TTI.
  // ---------------------------------------------------------------------------
  int estimateLoopBodyCost(Loop *L, TargetTransformInfo &TTI) {
    int Cost = 0;
    for (BasicBlock *BB : L->getBlocks()) {
      ...
      for (Instruction &I : *BB) {
        ...
        auto IC = TTI.getInstructionCost(&I,
  ...
  // ---------------------------------------------------------------------------
  // Check if a loop is safe to parallelize.
  // ---------------------------------------------------------------------------
  bool isLoopParallelizable(Loop *L, Function &F, ScalarEvolution &SE,
    ...
    // Keep it simple: bail on complex loops.
    ...
    LLVM_DEBUG(dbgs() << "Loop is safe to parallelize (no unsafe memory deps)\n");
    return true;
  }

  // ---------------------------------------------------------------------------
  // Parallelize a simple canonical loop using OpenMPIRBuilder.
  // ---------------------------------------------------------------------------
  bool parallelizeLoop(Loop *L, Function &F, ScalarEvolution &SE,
    ...
    auto CLIOrError = OMPBuilder.createCanonicalLoop(
        ...
        [&](IRBuilderBase::InsertPoint IP, Value *IV) -> llvm::Error {
          ...
          // Clone header body (skip PHIs/terminator)
          for (Instruction &I : *Header) {
            ...
            for (unsigned i = 0; i < ClonedI->getNumOperands(); ++i) {
              ...
              if (It != VMap.end())
                ClonedI->setOperand(i, It->second);
            }
            Builder.Insert(ClonedI);
            VMap[&I] = ClonedI;
          }

          // Clone other loop blocks (excluding header and latch)
          for (BasicBlock *BB : L->getBlocks()) {
  ...
  // ---------------------------------------------------------------------------
  // Profitability & legality check for fusion.
  // ---------------------------------------------------------------------------
  bool canFuseLoops(Loop *L1, Loop *L2, Function &F, ScalarEvolution &SE,
    ...
    // Simple loops only: header + latch (+ maybe exit).
    ...
    if (auto *BR = dyn_cast<BranchInst>(L1Exit->getTerminator())) {
      if (BR->isConditional() || BR->getSuccessor(0) != L2Preheader)
        return false;
    } else {
      return false;
    }

    // Conservative: don't fuse if L2 may read from L1's writes.
    for (BasicBlock *BB1 : L1->getBlocks()) {
      for (Instruction &I1 : *BB1) {
        if (auto *SI = dyn_cast<StoreInst>(&I1)) {
          Value *Ptr1 = SI->getPointerOperand();
          for (BasicBlock *BB2 : L2->getBlocks()) {
            for (Instruction &I2 : *BB2) {
              if (auto *LI2 = dyn_cast<LoadInst>(&I2)) {
                Value *Ptr2 = LI2->getPointerOperand();
                if (AA.alias(Ptr1, Ptr2) != AliasResult::NoAlias) {
                  LLVM_DEBUG(dbgs()
                             << "Loop fusion blocked: L2 reads from L1\n");
                  return false;
                }
              }
            }
    ...
    // Profitability: only fuse if combined cost isn't too big.
    int Cost1        = estimateLoopBodyCost(L1, TTI);
    int Cost2        = estimateLoopBodyCost(L2, TTI);
    int CombinedCost = Cost1 + Cost2;

    if (CombinedCost > 3 * std::min(Cost1, Cost2)) {
      LLVM_DEBUG(dbgs() << "Fusion not profitable: combined cost "
                        << CombinedCost << " too large vs individual\n");
      return false;
    }

    LLVM_DEBUG(dbgs() << "Loops can be fused (cost1=" << Cost1
                      << ", cost2=" << Cost2
  ...
  // ---------------------------------------------------------------------------
  // Simple loop info + fusion implementation
  // ---------------------------------------------------------------------------
  ...
  bool getSimpleLoopInfo(Loop *L, ScalarEvolution &SE, SimpleLoopInfo &Out) {
    (void)SE; // not used, but kept for signature symmetry
  ...
  bool fuseLoops(Loop *L1, Loop *L2, Function &F, ScalarEvolution &SE,
                 LoopInfo &LI, DominatorTree &DT) {
    (void)DT; // we mutate CFG but don't update DT here; pass returns none.
    ...
    auto cloneBlockIntoBody = [&](BasicBlock *BB) {
      for (Instruction &I : *BB) {
        if (isa<PHINode>(&I) || I.isTerminator())
          continue;
    ...
    Instruction *InsertPt = Latch1->getTerminator();
    Builder.SetInsertPoint(InsertPt);

    // Clone L2 header body into L1 loop.
    cloneBlockIntoBody(Header2);

    // Clone any other L2 blocks inside loop.
    for (BasicBlock *BB : L2->getBlocks()) {
      if (BB == Header2 || BB == Latch2 || BB == Exit2)
        continue;
      cloneBlockIntoBody(BB);
    }
...
llvmGetPassPluginInfo() {
  return {LLVM_PLUGIN_API_VERSION, "LoopParallelization", "0.1",
          [](PassBuilder &PB) {
