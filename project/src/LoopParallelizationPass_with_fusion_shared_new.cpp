  ...
  PreservedAnalyses run(Function &F, FunctionAnalysisManager &FAM) {
    ...
    for (Loop *L : LI) {
      if (isLoopParallelizable(L, F, SE, LI, DT, TTI, AA, AC, TLI)) {
        ...
        errs() << "Found parallelizable loop in function: "
               << F.getName() << "\n";
      }
    }

    // Step 4: Shared OpenMPIRBuilder for all loops in this function
...
private:
  // ---------------------------------------------------------------------------
  // Cost model: rough per-iteration cost using TTI.
  // ---------------------------------------------------------------------------
    ...
    for (BasicBlock *BB : L->getBlocks()) {
      ...
      for (Instruction &I : *BB) {
        if (isa<PHINode>(&I) || I.isTerminator())
          continue;
        auto IC = TTI.getInstructionCost(
  ...
  bool isLoopParallelizable(Loop *L, Function &F, ScalarEvolution &SE,
    ...
    // LoopAccessInfo ctor matches your LLVM version (as in your working code)
    ...
    LLVM_DEBUG(dbgs() << "Loop is safe to parallelize (no unsafe deps)\n");
    return true;
  }

  // ---------------------------------------------------------------------------
  // Loop parallelization using shared OpenMPIRBuilder + worksharing
  // ---------------------------------------------------------------------------
  ...
  bool canFuseLoops(Loop *L1, Loop *L2, Function &F, ScalarEvolution &SE,
    ...
    // Only very simple loops: header + latch (+ maybe exit)
    ...
    // Conservative: do NOT fuse if L2 reads from L1's writes
    ...
    // Profitability: only fuse if combined cost not too big
  ...
  bool getSimpleLoopInfo(Loop *L, ScalarEvolution &SE, SimpleLoopInfo &Out) {
    (void)SE; // not used here, but kept for symmetry
  ...
  bool fuseLoops(Loop *L1, Loop *L2, Function &F, ScalarEvolution &SE,
                 LoopInfo &LI, DominatorTree &DT) {
    (void)DT; // DT is stale after fusion; we return PreservedAnalyses::none().
