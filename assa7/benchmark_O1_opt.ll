...
define nofpclass(nan inf) float @loop_div_sum(ptr noundef readonly captures(none) %0, i32 noundef %1, float noundef nofpclass(nan inf) %2) local_unnamed_addr #0 {
  %4 = icmp sgt i32 %1, 0
...
7:                                                ; preds = %9, %3
  %8 = phi float [ 0.000000e+00, %3 ], [ %22, %9 ]
  ret float %8
...
9:                                                ; preds = %9, %5
  ...
  %24 = icmp eq i64 %23, %6
  br i1 %24, label %7, label %9, !llvm.loop !10
}
...
define nofpclass(nan inf) float @branchy(float noundef nofpclass(nan inf) %0, float noundef nofpclass(nan inf) %1, float noundef nofpclass(nan inf) %2) local_unnamed_addr #2 {
  ...
  %rcp = fdiv fast float 1.000000e+00, %8
  %div.mul = fmul fast float %7, %rcp
  ret float %div.mul
}
